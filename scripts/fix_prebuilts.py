...
if os.path.isfile(llvm_so):
    # Step A: Remove empty libLLVM_android from sdk/tools (use regex, robust)
    if os.path.isfile(sdk_tools_bp):
        with open(sdk_tools_bp, 'r') as f:
            content = f.read()
        # Match the entire libLLVM_android block (handles any whitespace)
        pattern = r'\n?cc_prebuilt_library_shared\s*\{\s*\n\s*name:\s*"libLLVM_android".*?\n\s*\}'
    ...
    # Step B: Always recreate clang-3289846/Android.bp (delete old first)
    bp_content = '''//
...
'''
    # Check if we need to write/update
    write_needed = True
    if os.path.exists(clang_bp):
        with open(clang_bp, 'r') as f:
            existing = f.read()
        if existing.strip() == bp_content.strip():
            write_needed = False
    
    if write_needed:
        with open(clang_bp, 'w') as f:
            f.write(bp_content)
        print('Created/updated libLLVM_android module at', clang_bp)
        changed += 1
