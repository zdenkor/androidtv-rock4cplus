...
# ---------------------------------------------------------------------------
# 10. Fix libLLVM_android module (exists but missing srcs in prebuilts/sdk/tools)
# ---------------------------------------------------------------------------
sdk_tools_bp = os.path.join(WORK_DIR, 'prebuilts', 'sdk', 'tools', 'Android.bp')
clang_llvm = os.path.join(WORK_DIR, 'prebuilts', 'clang', 'host', 'linux-x86', 'clang-3289846', 'lib64', 'libLLVM.so')

if os.path.isfile(sdk_tools_bp) and os.path.isfile(clang_llvm):
    with open(sdk_tools_bp, 'r') as f:
        content = f.read()
    
    # Check if libLLVM_android already has srcs
    if 'name: "libLLVM_android"' in content and 'srcs:' not in content.split('name: "libLLVM_android"')[1].split('name: "libclang_android"')[0]:
        # Add srcs and target to the existing libLLVM_android module
    ...
    native_bridge_supported: true,
        ...
        new_module = '''cc_prebuilt_library_shared {
    name: "libLLVM_android",
    vendor_available: true,
    host_supported: true,
    // TODO(ccross): this is necessary because the prebuilt module must have
    ...
    target: {
        linux_glibc_x86_64: {
            srcs: ["../../clang/host/linux-x86/clang-3289846/lib64/libLLVM.so"],
        },
    },
    strip: {
        none: true,
        ...
        if old_module in content:
            ...
            with open(sdk_tools_bp, 'w') as f:
                f.write(content)
            print('Fixed libLLVM_android module (added srcs) in', sdk_tools_bp)
            changed += 1
        else:
            print('libLLVM_android module format changed — skipping auto-fix')

# Remove duplicate Android.bp if we created one earlier
dup_bp = os.path.join(WORK_DIR, 'prebuilts', 'clang', 'host', 'linux-x86', 'clang-3289846', 'Android.bp')
if os.path.exists(dup_bp):
    os.remove(dup_bp)
    print('Removed duplicate', dup_bp)
    changed += 1
